...
const appActions = {
  connectWithFormInput: async () => {
    ...
    const roomOpts: RoomOptions = {
      ...
      e2ee: e2eeEnabled
        ? {
            keyProvider: state.e2eeKeyProvider,
            worker: new Worker(new URL("livevideo-client/e2ee-worker", import.meta.url)),
          }
        : undefined,
    };
    if (roomOpts.publishDefaults?.videoCodec === "av1" || roomOpts.publishDefaults?.videoCodec === "vp9") {
  ...
  connectToRoom: async (
    ...
    room
      ...
      .on(RoomEvent.Reconnected, async () => {
        appendLog("Successfully reconnected. server", await room.engine.getConnectedServerAddress());
      })
      ...
      .on(RoomEvent.TrackSubscribed, (_, pub, participant) => {
      ...
      .on(RoomEvent.TrackStreamStateChanged, (pub, streamState, participant) => {
        appendLog(`stream state changed for ${pub.trackSid} (${participant.identity}) to ${streamState.toString()}`);
      });
...
function populateSelect(element: HTMLSelectElement, devices: MediaDeviceInfo[], selectedDeviceId?: string) {
  // clear all elements
  element.innerHTML = "";
